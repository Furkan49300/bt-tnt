...
def create_and_print_shipment():
    try:
        ...
        # Vérifiez que les données importantes sont présentes
        if not data:
            raise ValueError("No data received in the request.")
        
        company_name = data.get('company_name')
        shipper_address = data.get('shipper_address', {})
        shipper_street1 = shipper_address.get('street1')
        shipper_street2 = shipper_address.get('street2')
        shipper_city = shipper_address.get('city')
        shipper_zip = shipper_address.get('zip')
        shipper_phone_number = shipper_address.get('phone')
        
        receipt_name = data.get('receipt_name')
        receipt_billing_address = data.get('receipt_billing_address', {})
        receipt_street1 = receipt_billing_address.get('street1')
        receipt_street2 = receipt_billing_address.get('street2')
        receipt_city = receipt_billing_address.get('city')
